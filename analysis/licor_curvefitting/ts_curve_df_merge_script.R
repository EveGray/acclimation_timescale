...
### Curve185_data
Curve185_data <- subset(aci.df, unique_id == aci.df.unique_id[185] & Ci < 1000) # find correct curve from full dataframe and make new object
...
### Curve190_data
Curve190_data <- subset(aci.df, unique_id == aci.df.unique_id[190] & Ci < 1000) # find correct curve from full dataframe and make new object
...
### Curve192_data
Curve192_data <- subset(aci.df, unique_id == aci.df.unique_id[192] & Ci < 1000) # find correct curve from full dataframe and make new object
...
### Curve273_data
Curve273_data <- subset(aci.df, unique_id == aci.df.unique_id[273] & Ci < 1000) # find correct curve from full dataframe and make new object
plot(Curve273_data$A~Curve273_data$Ci) # plot the A/Ci data and look for any weirdness, adjust as needed
Curve273_fit <- fitaci(Curve273_data, varnames = list(ALEAF = "A", # fit the curves
                                                      Tleaf = "Tleaf",
                                                      Ci = "Ci",
                                                      PPFD = "Qin"),
                       fitTPU = FALSE, Tcorrect = FALSE, useRd = FALSE)
summary(Curve273_fit) # take a look at fitted values, adjust as needed
plot(Curve273_fit) # plot the fitted curves over the raw data, adjust as needed
Curve273_output <- cbind('Curve273', Curve273_data$id[1], Curve273_data$unique_id[1], Curve273_data$machine[1], Curve273_data$baseline_yn[1],
                         Curve273_data$A[1], Curve273_data$Ci[1], Curve273_data$gsw[1],
                         mean(Curve273_data$VPDleaf, na.rm = T), mean(Curve273_data$Tleaf, na.rm = T), mean(Curve273_data$Qin, na.rm = T),
                         Curve273_fit[[2]][1,1], Curve273_fit[[2]][1,2],
                         Curve273_fit[[2]][2,1], Curve273_fit[[2]][2,2],
                         Curve273_fit[[2]][3,1], Curve273_fit[[2]][3,2],
                         Curve273_fit$RMSE,
                         Curve273_fit$Ci_transition,
                         Curve273_fit$citransition,
                         Curve273_fit$Km,
                         Curve273_fit$GammaStar,
                         Curve273_fit$fitmethod,
                         Curve273_fit$Tcorrect,
                         Curve273_fit$fitTPU) # put relevant data together in a vector of values
colnames(Curve273_output) <- c('curve_fit_number', 'id', 'unique_id', 'machine', 'baseline_yn',
                               'anet_420', 'ci_420', 'gs_420',
                               'vpd_leaf', 'temperature_leaf', 'par_leaf',
                               'vcmax_tleaf', 'vcmax_tleaf_se',
                               'jmax_tleaf', 'jmax_tleaf_se', 
                               ...
                               'aci_km', 'aci_gammastar', 'aci_fitmethod',
                               'aci_tcorrect', 'aci_fittpu') # add column names to the vector of values
all_curve_fits <- rbind(all_curve_fits, Curve273_output) # add the curve fits to the larger data frame

### Curve274_data
Curve274_data <- subset(aci.df, unique_id == aci.df.unique_id[274] & Ci < 1000) # find correct curve from full dataframe and make new object
plot(Curve274_data$A~Curve274_data$Ci) # plot the A/Ci data and look for any weirdness, adjust as needed
Curve274_fit <- fitaci(Curve274_data, varnames = list(ALEAF = "A", # fit the curves
                                                      Tleaf = "Tleaf",
                                                      Ci = "Ci",
                                                      PPFD = "Qin"),
                       fitTPU = FALSE, Tcorrect = FALSE, useRd = FALSE)
summary(Curve274_fit) # take a look at fitted values, adjust as needed
plot(Curve274_fit) # plot the fitted curves over the raw data, adjust as needed
Curve274_output <- cbind('Curve274', Curve274_data$id[1], Curve274_data$unique_id[1], Curve274_data$machine[1], Curve274_data$baseline_yn[1],
                         Curve274_data$A[1], Curve274_data$Ci[1], Curve274_data$gsw[1],
                         mean(Curve274_data$VPDleaf, na.rm = T), mean(Curve274_data$Tleaf, na.rm = T), mean(Curve274_data$Qin, na.rm = T),
                         Curve274_fit[[2]][1,1], Curve274_fit[[2]][1,2],
                         Curve274_fit[[2]][2,1], Curve274_fit[[2]][2,2],
                         Curve274_fit[[2]][3,1], Curve274_fit[[2]][3,2],
                         Curve274_fit$RMSE,
                         Curve274_fit$Ci_transition,
                         Curve274_fit$citransition,
                         Curve274_fit$Km,
                         Curve274_fit$GammaStar,
                         Curve274_fit$fitmethod,
                         Curve274_fit$Tcorrect,
                         Curve274_fit$fitTPU) # put relevant data together in a vector of values
colnames(Curve274_output) <- c('curve_fit_number', 'id', 'unique_id', 'machine', 'baseline_yn',
                               'anet_420', 'ci_420', 'gs_420',
                               'vpd_leaf', 'temperature_leaf', 'par_leaf',
                               'vcmax_tleaf', 'vcmax_tleaf_se',
                               'jmax_tleaf', 'jmax_tleaf_se', 
